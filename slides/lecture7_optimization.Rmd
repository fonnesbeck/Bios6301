...
Since we are searching for $x^∗$ such that $f′(x^∗) = 0$, we will use $|f′(x_i)| ≤ \epsilon$ as our stopping criterion.
...
While $x_b - x_a \gt \epsilon$:

* if $x_b - x_c \gt x_c - x_a$, choose $y \in (x_c, x_b)$:
...
* else if $x_b - x_c \lt x_c - x_a$, choose $y \in (x_a, x_c)$:
